...
export const getAllFollowups: RequestHandler = async (req, res) => {
  ...
  const userId = isManager ? undefined : user.id;
  const followups = await followupModel.getAllFollowups(userId, from, to);
...
export const getFollowupActivity: RequestHandler = async (req, res) => {
  const from = typeof req.query["from"] === "string" ? req.query["from"] : undefined;
  const to   = typeof req.query["to"]   === "string" ? req.query["to"]   : undefined;
  const summary = await followupModel.getFollowupActivitySummary(from, to);
