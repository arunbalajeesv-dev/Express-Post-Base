...
function FollowupCard({
  ...
  const isDueSoon  = isPending && (f.followupDate === today || f.followupDate === tomorrow);
    ...
    <div className="space-y-2">
      ...
      {open && (
        <div className="space-y-3">
          {followups.map((f) => (
            <FollowupCard key={f.id} f={f} onReport={() => onReport(f)} />
          ))}
        </div>
      )}
    </div>
...
export default function Followups() {
  ...
  const dueSoon   = filtered.filter((f) => f.status === "Pending" && (f.followupDate === today || f.followupDate === tomorrow));
  ...
  const todayOverdue    = all.filter((f) => f.status === "Pending" && f.followupDate <= today);
        ...
        <div className="flex items-start gap-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl px-4 py-3">
          ...
          <div>
            <p className="text-sm font-semibold text-red-700 dark:text-red-300">
              {todayOverdue.length} overdue follow-up{todayOverdue.length !== 1 ? "s" : ""}
            </p>
